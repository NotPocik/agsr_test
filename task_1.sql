...
/* 
В качестве СУБД использовал PostgreSQL 15.
...
WHERE gasoline_consumption IS NULL) / COUNT(*) * 100 as null_percentage
FROM test.car_models;
...
-- Здесь будет дублирование модели, если клиент купил одну модель более одного раза. Если такое не нужно,
-- просто всунуть DISTINCT в STRING_AGG.
SELECT 
	c.customer_id,
	person_name,
...
но она принадлежит автомобилю Porsche, для которых здесь страна по какой-то причине не определена
